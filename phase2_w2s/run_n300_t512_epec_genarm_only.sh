#!/bin/bash
# Extend EPEC+GenARM from n=200 -> n=300 (add 100 new prompts × 6 α = 600 new prompts).
# Reuses existing 6 GenARM dirs at results_phase2_n50_t512_epec/genarm/ via
# --resume true: uids 0-199 already in generation.json are skipped, only uids
# 200-299 get newly generated.
#
# Re-scores all 6 dirs with Beaver afterwards (so mean_result.json reflects n=300).
#
# Estimated runtime: 600 prompts × ~120s avg = ~20h on A100.
# max_tokens=512.
#
# Prereqs on a100-demo:
#   source ~/common-agency/venv/bin/activate
#   tmux new -s phase2_n300_genarm
#   bash ~/phase2_w2s/run_n300_t512_epec_genarm_only.sh 2>&1 | tee ~/phase2_n300_genarm.log
...
OUT_ROOT="$REPO/results_phase2_n50_t512_epec"   # SAME root as n=50/200 — extend per-config dirs
...
############# 3. BEAVER scoring (re-score on n=300 superset) #############
echo ""
echo "=== [3/3] Beaver-7B reward + cost scoring (6 GenARM configs at n=300) ==="
for AH in "${ALPHAS[@]}"; do
    AS=$(python -c "print(round(1.0 - $AH, 1))")
