...
def load_models(args, device):
    ...
    if args.base.endswith("GPTQ") or args.base.endswith("GGML"):
        ...
        base_model = AutoGPTQForCausalLM.from_quantized(
            ...
            device_map={"": 0},
            # NOTE: tried use_marlin=True; per-prompt fwd time was unchanged
            # but EPEC inner-solve time grew 7x (21s -> 157s/prompt). Suspect
            # Marlin's logit output memory layout hits a slow numpy/scipy path.
            # Disabled until diagnosed. See commit 48e140c for the patch.
        )
    else:
