cuda=0
exp_name=mod_harm_r64_2epoch
objective=harm

# MOD paper hyperparameters (Shi et al., 2024)
lora_r=64
lora_alpha=1
beta=0.1

epoch=2
learning_rate=5e-4
bs=32
per_device_train_batch_size=4

model_name_or_path=PKU-Alignment/alpaca-7b-reproduced

num_GPU=$(echo $cuda | awk -F, '{print NF}')
gradient_accumulation_steps=$(($bs/$num_GPU/$per_device_train_batch_size))
preference_dataset=PKU_SafeRLHF

output_dir=./exp_mod_harm
if [ -d "${output_dir}" ]; then
    echo -e "\n\n"
    echo "Error: Directory "${output_dir}" already exists. Please delete it or choose a new output_dir." >&2
    exit 1
fi
echo "Output dir: $output_dir"

accelerate launch --gpu_ids $cuda --main_process_port 29503 --num_processes $num_GPU train_genarm.py \
    --preference_dataset=$preference_dataset \
    --objective=$objective \
    --lora_r=$lora_r \
    --lora_alpha=$lora_alpha \
    --lora_dropout=0.0 \
    --model_name_or_path=$model_name_or_path \
    --beta=$beta \
    --learning_rate=$learning_rate \
    --num_train_epochs=$epoch \
    --output_dir=$output_dir \
    --run_name=$exp_name \
    --per_device_train_batch_size=$per_device_train_batch_size \
    --gradient_accumulation_steps=$gradient_accumulation_steps \
    --per_device_eval_batch_size=2 \
    --logging_steps=10 \
    --evaluation_strategy="steps" \
    --eval_steps=20 \
    --save_strategy="steps" \
    --save_steps=1000 \
    --lr_scheduler_type="cosine" \
    --warmup_steps=20 \
    --weight_decay=0.05 \
    --gradient_checkpointing=True \
    --bf16=True \
    --max_prompt_length=512 \
    --max_length=1024 \
    --report_to="none" \
    --remove_unused_columns=False

echo "Finished training $output_dir"
